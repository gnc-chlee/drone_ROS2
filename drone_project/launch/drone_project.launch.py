...
def generate_launch_description():
    ...
    # turtlebot_control 노드 실행
    turtlebot_control_node = Node(
        package='drone_project',
    ...
    # rqt_reconfigure 실행
    rqt_reconfigure_cmd = ExecuteProcess(
    ...
    nodes_to_run = [
        ...
        TimerAction(
            ...
            actions=[
                ...
                TimerAction(
                    period=2.0,  # 2초 대기 (필요 시 조정)
                    actions=[
                        camera_display_node,  # 드론 제어 노드
                        TimerAction(
                            period=2.0,  # 2초 대기 (필요 시 조정)
                            actions=[
                                turtlebot_control_node,  # 터틀봇 제어 노드
                                TimerAction(
                                    period=2.0,  # 2초 대기 (필요 시 조정)
                                    actions=[rqt_reconfigure_cmd]  # rqt_reconfigure 실행
                                )
                            ]
                        )
                    ]
